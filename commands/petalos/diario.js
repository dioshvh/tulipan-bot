...
module.exports = {
    ...
    try {
        ...
        if (lastDaily !== null && cooldown - (Date.now() - lastDaily) > 0) {
            ...
            return message.channel.send(`Lo siento, no puede recoger sus <a:Roses:832495000804786176> Pétalos diarios. Por favor, espere ${finalTime}.`);
        } else {
            db.set(`lastDaily.${message.author.id}`, Date.now());
            db.add(`account.${message.author.id}.balance`, amount);
            return message.channel.send(`Genial **${message.author.tag}!** Usted ha recibido <a:Roses:832495000804786176> 500 Pétalos!`);
        }
